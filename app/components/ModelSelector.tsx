...
export default function ModelSelector({ onModelChange }: ModelSelectorProps) {
  ...
  const playElasticAnimation = useCallback(() => {
  ...
  return (
    <View style={styles.wrapper}>
      <Pressable onPress={handleOpenMenu}>
        <Animated.View
          ref={buttonRef}
          style={[styles.iconRow, { transform: [{ scale: iconScale }] }]}>
          <ModelIcon option={currentOption} size={25} />
  ...
  if (imgError) {
    return (
      <View style={[styles.fallbackIcon, { width: size, height: size, borderRadius: size / 2 }]}>
        <Text style={[styles.fallbackText, { fontSize: size * 0.5 }]}>{option.fallbackIcon}</Text>
      </View>
    );
  }
